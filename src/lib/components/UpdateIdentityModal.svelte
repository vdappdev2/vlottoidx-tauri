<script lang="ts">
  ...
  async function loadAddresses() {
    ...
    try {
      ...
      try {
        ...
        console.log('[UpdateIdentityModal]: Loading primary addresses for chain:', connectionState?.selectedChain, 'param:', chainParam);
  ...
  async function loadSourceFundsAddresses() {
    ...
    try {
      ...
      try {
        ...
        const chainParam = getChainParam(connectionState?.selectedChain);
        console.log('[UpdateIdentityModal]: Loading transparent addresses for source funds for chain:', connectionState?.selectedChain, 'param:', chainParam);
      ...
      try {
        ...
        const chainParam = getChainParam(connectionState?.selectedChain);
        console.log('[UpdateIdentityModal]: Loading private addresses for source funds for chain:', connectionState?.selectedChain, 'param:', chainParam);
  ...
  async function loadPrivateAddresses() {
    ...
    try {
      ...
      const chainParam = getChainParam(connectionState?.selectedChain);
      console.log('[UpdateIdentityModal]: Loading private addresses for chain:', connectionState?.selectedChain, 'param:', chainParam);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const params: any = {
        jsonIdentity,
        returnTx: false,
        tokenUpdate: formData.tokenUpdate
      };
      ...
      const chainParam = getChainParam(connectionState?.selectedChain);
      console.log('[UpdateIdentityModal]: Updating identity for chain:', connectionState?.selectedChain, 'param:', chainParam);
      const result = await invoke('update_identity', { ...params, chain: chainParam });
      console.log('✅ Identity updated:', result);
      ...
      // Show success toast
      showSuccess('Success: identity updated', { txid: successTxId });
      ...
      error = typeof err === 'string' ? err : 'Failed to update identity';
      showError(error);
    } finally {
      isSubmitting = false;
    }
...
>
  <form onsubmit={(e) => { e.preventDefault(); if (isFormValid) handleSubmit(); }}>
    <div class="space-y-4">
      ...
      <div>
        ...
        <button
          ...
          class="mt-2 text-sm text-verusidx-turquoise-deep hover:text-verusidx-turquoise-bright disabled:opacity-50"
        >
      ...
      <div>
        <label class="flex items-center space-x-2 cursor-pointer">
          <input
            type="checkbox"
            bind:checked={formData.tokenUpdate}
        ...
        <p class="mt-1 ml-6 text-xs text-verusidx-mountain-grey dark:text-verusidx-mountain-mist">
          Use the tokenized ID control token to update (if one exists). This enables changing the revocation or recovery IDs even without controlling them.
        </p>
      </div>
    ...
    <div class="mt-6 flex justify-end space-x-3">
      ...
      >
        {isSubmitting ? 'Updating...' : 'Update Identity'}
      </button>
    </div>
  </form>
</Modal>