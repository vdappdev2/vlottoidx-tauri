<script lang="ts">
  ...
  // Form state for centralized basket currency
  let formData = $state({
    name: '',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      console.log('CentralizedBasketModal: Defining currency on chain:', connectionState?.selectedChain, 'param:', chainParam);
      ...
      console.log('Centralized basket currency created successfully:', sendResult);
      ...
      console.error('Centralized basket currency creation failed:', err);
...
<Modal {isOpen} onclose={onClose} title="Create Centralized Basket Currency" size="xl">
  <div class="p-6">
    <!-- Centralized Basket Currency Info -->
    <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
      <p class="text-blue-700 dark:text-blue-300 text-sm">
        🏦 <strong>Centralized Basket Currency:</strong> AMM currency you can mint/burn while controlling reserve ratios. 
    ...
    <div class="flex space-x-4 pt-6 border-t border-verusidx-mountain-mist dark:border-verusidx-stone-medium mt-6">
      ...
      >
        {isSubmitting ? 'Creating Centralized Basket...' : 'Create Centralized Basket'}
      </button>
    </div>
  </div>
</Modal>