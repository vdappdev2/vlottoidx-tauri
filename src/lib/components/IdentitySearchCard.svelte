<script lang="ts">
  ...
  async function handleSearch() {
    ...
    try {
      ...
      searchError = typeof error === 'string' ? error : "Failed to find identity. Please check the name and try again.";
      searchResults = null;
      hasSearched = true;
    } finally {
      isSearching = false;
  ...
  function searchRecent(identity: string) {
  ...
  function handleChainChanged(event: CustomEvent) {
    console.log("IdentitySearchCard: Chain changed to", event.detail.chainName);
...
<div class="bg-white dark:bg-verusidx-stone-dark rounded-lg shadow-xl">
  ...
  <div class="p-6 border-b border-verusidx-mountain-mist dark:border-verusidx-stone-medium">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl font-semibold text-verusidx-stone-dark dark:text-white">Identity Lookup</h2>
      {#if hasSearched}
        <button
          onclick={clearSearch}
    ...
    <div class="relative">
      <div class="flex">
        <input
          ...
          placeholder="Search any identity@ on the network..."
  ...
  <div class="p-6">
    {#if isSearching}
      <div class="flex items-center justify-center py-8">
        ...
        <span class="ml-3 text-verusidx-mountain-grey dark:text-verusidx-mountain-mist">Searching identity...</span>
      </div>
    {:else if searchError}
      <div class="text-center py-8">
        ...
        <p class="text-sm text-verusidx-mountain-grey dark:text-verusidx-mountain-mist mt-2">
          Make sure the identity name is correct and includes the @ symbol
        </p>
      </div>
    {:else if hasSearched && searchResults}
      <div class="space-y-4">
        ...
        <IdentityDetailsView identityData={searchResults} />
      </div>
    {:else if hasSearched && !searchResults}
      <div class="text-center py-8">
        ...
        <p class="text-sm text-verusidx-mountain-grey dark:text-verusidx-mountain-mist mt-2">
          The identity "{searchQuery}" could not be found on the network
        </p>
      </div>
    {:else}
      <div class="text-center py-8">
        ...
        <p class="text-verusidx-mountain-grey dark:text-verusidx-mountain-mist font-medium">Search for any identity on the network</p>
        <p class="text-sm text-verusidx-mountain-grey dark:text-verusidx-mountain-mist mt-2">
          Enter an identity name (e.g., "mike@") to view its details
        </p>
      </div>
    {/if}
  </div>
